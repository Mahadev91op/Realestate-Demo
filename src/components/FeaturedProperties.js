...
// --- Sub Component for Card (Reusable) ---
function PropertyCard({ property }) {
  return (
    <div className="group bg-transparent h-full">
      <div className="relative h-full bg-white rounded-2xl md:rounded-[2rem] overflow-hidden border border-gray-100 shadow-lg md:shadow-xl transition-all duration-300 ease-out hover:-translate-y-1 md:hover:-translate-y-2 hover:shadow-xl md:hover:shadow-2xl hover:shadow-blue-500/10 will-change-transform flex flex-col">
        
        {/* Image Section */}
        <div className="relative h-48 md:h-[300px] w-full shrink-0 overflow-hidden">
          <Image 
            src={property.image} 
            alt={property.title} 
            fill 
          ...
          <div className="absolute bottom-3 left-3 right-3 md:bottom-5 md:left-5 md:right-5 text-white">
            ...
            <div className="flex items-center justify-between">
                <h3 className="text-sm md:text-lg font-bold truncate pr-2">{property.title}</h3>
                  {/* Agent Avatar */}
        ...
        <div className="p-3 md:p-5 flex flex-col grow justify-between">
          ...
          <div className="grid grid-cols-3 gap-2 md:gap-3 mb-3 md:mb-5">
            <div className="flex flex-col items-center justify-center p-1.5 md:p-2 rounded-lg md:rounded-xl bg-gray-50 border border-gray-100 group-hover:border-blue-100 group-hover:bg-blue-50/50 transition-colors">
              <Bed size={14} className="md:w-[18px] md:h-[18px] text-gray-400 group-hover:text-blue-500 mb-0.5 md:mb-1 transition-colors" />
              <span className="text-[9px] md:text-xs font-bold text-gray-700">{property.beds} Beds</span>
            </div>
            <div className="flex flex-col items-center justify-center p-1.5 md:p-2 rounded-lg md:rounded-xl bg-gray-50 border border-gray-100 group-hover:border-blue-100 group-hover:bg-blue-50/50 transition-colors">
              <Bath size={14} className="md:w-[18px] md:h-[18px] text-gray-400 group-hover:text-blue-500 mb-0.5 md:mb-1 transition-colors" />
              <span className="text-[9px] md:text-xs font-bold text-gray-700">{property.baths} Baths</span>
            </div>
            <div className="flex flex-col items-center justify-center p-1.5 md:p-2 rounded-lg md:rounded-xl bg-gray-50 border border-gray-100 group-hover:border-blue-100 group-hover:bg-blue-50/50 transition-colors">
              <Square size={14} className="md:w-[18px] md:h-[18px] text-gray-400 group-hover:text-blue-500 mb-0.5 md:mb-1 transition-colors" />
              <span className="text-[9px] md:text-xs font-bold text-gray-700">{property.sqft}</span>
            </div>
          </div>

          {/* Action Button */}
          <button className="w-full py-2 md:py-3.5 bg-gray-900 text-white rounded-lg md:rounded-xl font-bold text-[10px] md:text-sm hover:bg-blue-600 transition-all duration-300 shadow-lg hover:shadow-blue-500/25 flex items-center justify-center gap-2 group/btn">
            View Details 
            <ArrowRight size={12} className="md:w-4 md:h-4 group-hover/btn:translate-x-1 transition-transform" />
          </button>
...
export default function FeaturedProperties() {
  ...
  // Split Logic: First 3 in Line 1, Rest in Line 2
  const row1Properties = filteredProperties.slice(0, 3);
  const row2Properties = filteredProperties.slice(3);
  ...
  return (
    <section className="py-12 md:py-24 bg-gray-50 relative overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        {/* --- MOBILE VIEW: Double Horizontal Scroll --- */}
        <div className="md:hidden flex flex-col gap-6">
            
            {/* ROW 1: Always Visible (First 3 items) */}
            <div className="flex overflow-x-auto gap-4 px-2 snap-x snap-mandatory scroll-smooth [&::-webkit-scrollbar]:hidden [-ms-overflow-style:'none'] [scrollbar-width:'none']">
                {row1Properties.map((property) => (
                    // Yahan width ko 70vw kiya hai taaki next card dikhe
                    <div key={property.id} className="min-w-[70vw] sm:min-w-[40%] snap-center">
                        <PropertyCard property={property} />
                    </div>
                ))}
            </div>

            {/* ROW 2: Visible Only if items > 3 */}
            {row2Properties.length > 0 && (
                <div className="flex overflow-x-auto gap-4 px-2 snap-x snap-mandatory scroll-smooth [&::-webkit-scrollbar]:hidden [-ms-overflow-style:'none'] [scrollbar-width:'none']">
                    {row2Properties.map((property) => (
                        // Yahan width ko 70vw kiya hai taaki next card dikhe
                        <div key={property.id} className="min-w-[70vw] sm:min-w-[40%] snap-center">
                            <PropertyCard property={property} />
                        </div>
        ...
        {/* --- PC VIEW: Standard Grid (Unchanged) --- */}
